...
from utils.config import SIMULATION_CORRTOEXP_TRAIN_OUTPUT_FOLDER, SORTED_FILENAMES_PATH
from utils.config import LATENT_OUTPUT_SIMCORRTOEXP 

from utils.preprocess import preprocess_simulation_output_data, load_sorted_filenames
...
path_output = SIMULATION_CORRTOEXP_TRAIN_OUTPUT_FOLDER  # for experimental images
...
batch_size = 32   # A5000 24GB trying for max usage without OOM
...
for chunk_start in range(0, total_files, chunk_size):
    ...
    output_data = preprocess_simulation_output_data(path_output, 0, len(chunk_filenames), img_filenames=chunk_filenames)
...
with open(LATENT_OUTPUT_SIMCORRTOEXP,'wb') as file:
    pickle.dump(image_np,file)

print(f"Saved latents to: {LATENT_OUTPUT_SIMCORRTOEXP}")
