...
% how many sims per SLURM task
SimsPerTask = 10; % testing things now

% get taskID from SLURM (fallback for local testing)
...
%     param_vector_3=[0,1,6.9512,216.9458,1.4860,1.1562,0.0548,10];
    param_vector_3=[0,1,9,160,1.2,0.8,0.05,7.5];
...
for t=1:SimsPerTask 
   ...
   % global simulation ID: unique across all tasks
    global_id = (taskID - 1)*SimsPerTask + t;   % 1,2,3,... up to ~100k

    % RNG: deterministic, no clock, no sync problems
    s1 = RandStream('mt19937ar','Seed',global_id);
    RandStream.setGlobalStream(s1);

    % also use this as the unique filename tag
    newnumber = global_id;
...
%              pick a random number between 1 and 20  % changed 050924 so
%              as to be similar as exps+ ro=5 patterns are just big
%              circular blobs 
                        no_of_seeds=randi([1,20],1);
        ...
        r_index=randsample(1:rand_mat_size,no_of_seeds);
...
%     Pattern_Generator(param_vector_1,Pattern,newnumber,tag1)
...
function Pattern_Generator(param_vector,Pattern1,newnumber,tag)



load('Parameters_multiseeding.mat'); % select parameter file
config  = 16; % select seeding configuration (see below)
...
%         t=1;
        ...
        % 
           ...
           base_name_output='/hpc/group/youlab/ks723/storage/MATLAB_SIMS/Sim_111425';

%             base_name_output='C:\Users\kinsh\Documents\MATLAB\OptimalPatterns_NanLuo-master\OptimalPatterns_NanLuo-master\Pattern_prediction\Sim_050924';
% 
    ...
    %         base_name_timepoint=strcat('Tp',num2str(i));
              ...
              base_name_time=strcat("Tp",num2str(i));

            base_name_output=fullfile(base_name_output,'Sim_input',base_name_complexity,base_name_time);
        ...
        %     base_name_output='/hpc/group/youlab/ks723/storage/MATLAB_SIMS/Sim_040423_100pointgrid/Sim_input'; 
            filename_output=strcat('Output','_',num2str(time_month),'_',num2str(time_day),'_',num2str(newnumber),'.png')  ; % here t is the value of iteration 
...
%     %         base_name_input='C:\Users\kinsh\Documents\MATLAB\OptimalPatterns_NanLuo-master\OptimalPatterns_NanLuo-master\Pattern_prediction\Sim_output_simple';
    ...
    %         base_name_output=fullfile(base_name_input,base_name_timepoint);
            ...
            base_name_input='/hpc/group/youlab/ks723/storage/MATLAB_SIMS/Sim_111425/Sim_output';
%               base_name_input='C:\Users\kinsh\Documents\MATLAB\OptimalPatterns_NanLuo-master\OptimalPatterns_NanLuo-master\Pattern_prediction\Sim_050924\Sim_output';
            ...
            filename_input=strcat('Input','_',num2str(time_month),'_',num2str(time_day),'_',num2str(newnumber),'.png');
